...
The entire program is **one line** of Embedded Python — **516 characters** — living in a single ObjectScript class method. It reads the 20 gzipped input files, extracts and filters the flux arrays, computes the percentage change, and writes the sorted CSV, without ever leaving that one expression.
...
- **`Flux.cls` is 516 characters** (the Embedded Python method body is 463 characters; the rest is the class/method scaffolding), down from an initial 896.
